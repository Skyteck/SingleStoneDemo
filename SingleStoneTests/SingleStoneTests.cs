...
{
    [TestClass]
    public class SingleStoneTests
    {

        private ContactsController SetupController()
        {
            ContactsController controller = new ContactsController();

            List<ContactModel> contacts = GetTestContacts();

            using (var db = new LiteDatabase(@"C:\Temp\MyDataTest.db"))
            {
        ...
        public void GetAllContacts_ShouldReturnAllContacts()
        {
            ...
            ContactsController controller = SetupController();
        ...
        [TestMethod]
        public void GetContactByID_ShouldOneContact()
        {
            //create the list of test contacts
            List<ContactModel> testContacts = GetTestContacts();
            //create the controller and populate the test DB
            ...
            //call the controller to get the contact
            var TheContact = controller.Get(2) as OkNegotiatedContentResult<ContactModel>;
            Assert.IsNotNull(TheContact);


            Assert.AreEqual("terry", TheContact.Content.name.first);
        }
